...
- **Widget refresh-in-progress indicator**: The widget refresh icon is now replaced with a static clock-loader icon (non-tappable) immediately when the user taps refresh, giving instant visual feedback while `TrmnlWidgetRefreshWorker` runs. The icon reverts to the normal refresh button once the worker completes (success, error, or retry).
- **Widget configuration disables devices without API key**: Devices that do not have a Device API key configured are now shown as disabled (dimmed) in the widget configuration screen. The subtitle now reads "ID: … · Device API key not configured" for those entries, and they cannot be selected until the key is set in the app.

### Fixed
- **Widget periodic refresh stops after first successful fetch**: Fixed `TrmnlWidgetRefreshWorker` using `ExistingWorkPolicy.KEEP` when self-rescheduling the next periodic refresh. WorkManager treats `RUNNING` state as "work already present" under `KEEP`, so the new delayed job was silently dropped every time the worker called `enqueue()` before returning `Result.success()`. After the first successful fetch, no follow-up job was ever queued and periodic refresh stopped entirely. Changed to `ExistingWorkPolicy.APPEND_OR_REPLACE`, which chains the new delayed job as a child of the current run without cancelling it.
- **Widget display image not loading on first add**: Fixed the widget remaining permanently in "Loading…" state after being added for the first time. On many OEM Android devices (Samsung, Xiaomi, etc.) with aggressive process management, the app process is killed before WorkManager can execute the initial `TrmnlWidgetRefreshWorker`, leaving the widget stuck with no image. The configuration activity now performs an **inline image fetch** while still in the foreground (before returning `RESULT_OK`), so the widget shows the display image immediately upon being placed on the home screen. If the inline fetch fails (no device token, network error, timeout) the activity still completes and the scheduled WorkManager job retries in the background as before. Also fixed a secondary issue where `TrmnlWidgetRefreshWorker` was silently returning `Result.success()` when the Glance widget ID could not be found — it now returns `Result.retry()` so a timing race on first add does not permanently stall the widget.
- **Widget stays on Loading after inline fetch**: Fixed a Glance session deduplication issue where the widget still showed "Loading…" even after the inline image fetch succeeded. Glance uses `ExistingWorkPolicy.KEEP` for session workers, so `update()` calls made while a session is already active are silently dropped. The active session (started by the initial `onUpdate`) captured `bitmap = null` at `provideGlance` invocation time, and state changes to `IMAGE_FILE_PATH_KEY` only triggered reactive recompositions — which still used the captured null bitmap. Fixed by moving bitmap decoding inside `provideContent` using `LaunchedEffect(imageFilePath)`, making the bitmap fully reactive to state changes without requiring a new `provideGlance` invocation.
...
### Changed
- **Circuit best practices**: Updated Circuit dependency from `0.30.0` to `0.33.1` and aligned code with Circuit best practices
  - Upgraded all `sealed class Event : CircuitUiEvent` to `sealed interface Event : CircuitUiEvent` across all screens for improved Compose compatibility
  - Converted inner sealed state classes (`DownloadState`, `RefreshState` in `DevicePreviewScreen`) to `sealed interface`
  - Converted `RecipesAnalyticsState` sealed class to `sealed interface`
...
- **Visual badges for device color capabilities in catalog**: Added color capability badges to device list items to help users quickly identify device characteristics at a glance
  - Badge types: Full Color (>1000 colors), Grayscale (17-1000 colors), Multi-tone (4-16 colors), Monochrome (2-3 colors)
  ...
  - Added comprehensive unit tests for color capability categorization logic (15 tests, all passing)
...
- **Added IntelliJ IDEA keybindings extension** - Pre-installed `k--kato.intellij-idea-keybindings` extension for developers familiar with IntelliJ IDEA keyboard shortcuts
...
- **Standardized test error messages** - Fixed TODO() statements in DeviceCatalogPresenterTest and standardized NotImplementedError messages across test files to use "Not needed for [SpecificTest] tests" format for better clarity and consistency
...
- **Refactored API test base class** - Created `BaseApiTest` base class to eliminate duplicate MockWebServer and Retrofit setup across 7 API test files, reducing ~50 lines of boilerplate per test file and improving test maintainability
