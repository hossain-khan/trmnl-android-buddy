...
interface TrmnlApiService {
    ...
    /**
     * Get a list of all supported TRMNL device models.
     *
     * Returns specifications for all compatible e-ink devices including:
     * - Official TRMNL devices (kind: "trmnl")
     * - Amazon Kindle e-readers (kind: "kindle")
     * - BYOD (Bring Your Own Device) compatible displays (kind: "byod")
     *
     * Requires authentication via Bearer token.
     *
     * @param authorization Bearer token with format "Bearer user_xxxxxx"
     ...
     *         val models = result.value.data
     *         val trmnlModels = models.filter { it.kind == "trmnl" }
     *         println("Found ${trmnlModels.size} TRMNL models")
     *     }
     *     is ApiResult.Failure.HttpFailure -> println("HTTP error: ${result.code}")
     *     is ApiResult.Failure.NetworkFailure -> println("Network error")
     *     is ApiResult.Failure.ApiFailure -> println("API error: ${result.error}")
